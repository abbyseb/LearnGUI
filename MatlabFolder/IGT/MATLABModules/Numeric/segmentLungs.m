function lungMask = segmentLungs(img,spacing,lungConnect,extConstraint)
%% lungMask = segmentLungs(img,spacing,lungConnect,extConstraint)
% ------------------------------------------
% FILE   : segmentLungs.m
...
% ------------------------------------------
% PURPOSE
%   Segment the lungs from an input 3D image.
% ------------------------------------------
% INPUT
...
%                   vector) in mm.
%                   Default: [1,1,1]
%   lungConnect:    To impose connectivity constrain on the lungs.
%                   (Optional) Default: true
%   extConstraint:  Use exterior constraint. This helps when sometimes the
%                   lungs are connected with the exterior region.
%                   (Optional) Default: false
%
% OUTPUT
%   lungMask:       The calculated lung mask.
...
if extConstraint
    ext = findExterior(ext);
end

if lungConnect
    lungMask = findConnected(~body & ~ext);
else
    lungCandidate = bwconncomp(~body & ~ext);
    candSize = cellfun(@length,lungCandidate.PixelIdxList);
    [~,ind] = sort(candSize);
    lungMask = false(size(body));
    for nc = 1:2
        for k = 1:length(lungCandidate.PixelIdxList{ind(end-nc+1)})
            lungMask(lungCandidate.PixelIdxList{ind(end-nc+1)}(k)) = true;
        end
    end
end
end