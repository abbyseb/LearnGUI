...
#ifndef __rtkPICCSConeBeamReconstructionFilter_h
#define __rtkPICCSConeBeamReconstructionFilter_h
...
/** \class PICCSConeBeamReconstructionFilter
 * \brief Implements the PICCS reconstruction
 *	f_obj = 0.5*|Rx-y|^2 + lambda * ( alpha * TV(x-xp) + (1-alpha) * TV(x) )
 *
 * \author Andy Shieh 2014
 *
 * \ingroup ReconstructionAlgorithm
 */
template<class TInputImage, class TOutputImage=TInputImage>
class ITK_EXPORT PICCSConeBeamReconstructionFilter :
  public rtk::IterativeConeBeamReconstructionFilter<TInputImage, TOutputImage>
{
public:
  /** Standard class typedefs. */
  typedef PICCSConeBeamReconstructionFilter                                Self;
...
/** Standard New method. */
  ...
  itkTypeMacro(PICCSConeBeamReconstructionFilter, IterativeConeBeamReconstructionFilter);
  ...
  /** Set/Get the truncation calibration map - Andy */
  itkSetMacro(TruncationCalibrationMap, InputImagePointerType);
  itkGetConstMacro(TruncationCalibrationMap, InputImagePointerType);
...
protected:
  ...
  // Truncation calibration multiplication filter
  typename MultiplyFilterType::Pointer				m_TruncationCalibrationFilter;
  ...
  // Calibration map for truncation mismatch correction
  InputImagePointerType	m_TruncationCalibrationMap;
...
private:
  ...
  PICCSConeBeamReconstructionFilter(const Self&);
...
#include "rtkPICCSConeBeamReconstructionFilter.txx"
#endif
