...
::BeforeThreadedGenerateData()
{
  ...
  if( this->GetUseConstantAngularGap() )
    {
	for(unsigned int k = 0; k < m_Geometry->GetTiltAngles().size(); ++k)
	  m_ConstantProjectionFactor.push_back( itk::Math::pi / 360 );
	}
  else if( m_Geometry->GetIsShortScan() )
	m_ConstantProjectionFactor = m_Geometry->GetShortScanAngularGaps( m_Geometry->GetSourceAngles() );
  else if( m_Geometry->GetIsDTS() )
    m_ConstantProjectionFactor = m_Geometry->GetDTSAngularGaps( m_Geometry->GetSourceAngles() );
  else
