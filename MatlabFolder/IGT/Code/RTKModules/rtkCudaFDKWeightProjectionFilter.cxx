...
::GPUGenerateData()
{
  ...
  if( this->GetUseConstantAngularGap() )
    {
	for(unsigned int k = 0; k < this->GetGeometry()->GetTiltAngles().size(); ++k)
	  constantProjectionFactor[k] = itk::Math::pi / 360.;
	}
  else if( this->GetGeometry()->GetIsShortScan() )
	constantProjectionFactor = this->GetGeometry()->GetShortScanAngularGaps( this->GetGeometry()->GetSourceAngles() );
  else if( this->GetGeometry()->GetIsDTS() )
    constantProjectionFactor = this->GetGeometry()->GetDTSAngularGaps( this->GetGeometry()->GetSourceAngles() );
  else
    constantProjectionFactor = this->GetGeometry()->GetAngularGaps( this->GetGeometry()->GetSourceAngles() );
