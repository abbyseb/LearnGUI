...
void
CudaWarpBackProjectionImageFilter
::SetDisplacementField(DVFType::Pointer DVF)
{
	this->SetInput("DisplacementField", DVF);
}
...
{
	if (!m_CudaDeformation)
		std::cerr << "CudaDeformation has not been set for rtkCudaWarpBackProjectionImageFilter" << std::endl;
...
//  const GeometryPointer geometry = dynamic_cast<GeometryType *>(this->GetGeometry().GetPointer() );
  ...
  for (unsigned int i = 0; i<nProj; ++i)
    ...
    //projectionSize[2] = std::min(nProj-i, (unsigned int)SLAB_SIZE);
	  ...
	  m_CudaDeformation->SetFrame(i);
	  m_CudaDeformation->Update();
	  this->SetDisplacementField(m_CudaDeformation->GetOutput());
	  pDVF = *(float**)(this->GetDisplacementField()->GetCudaDataManager()->GetGPUBufferPointer());
