...
{
  if(itksys::SystemTools::Strucmp(name, "IsShortScan") == 0)
    {
    m_IsShortScan = ( atof(this->m_CurCharacterData.c_str() ) != 0 );
	m_OutputObject->SetIsShortScan( m_IsShortScan );
	}
	
  if(itksys::SystemTools::Strucmp(name, "ShortScanAngle") == 0)
    {
    m_ShortScanAngle = atof(this->m_CurCharacterData.c_str() );
	m_OutputObject->SetShortScanAngle( m_ShortScanAngle );
	}
...
WriteFile()
{
  ...
  // Andy 2015: write short scan tag and angle
  // Write value in file if not 0.
    std::string ss("IsShortScan");
    output << indent;
    this->WriteStartElement(ss, output);
    output << ( this->m_InputObject->GetIsShortScan() );
    this->WriteEndElement(ss,output);
    output << std::endl;
  
    std::string ssa("ShortScanAngle");
    output << indent;
    this->WriteStartElement(ssa, output);
    output << ( this->m_InputObject->GetShortScanAngle() );
    this->WriteEndElement(ssa,output);
    output << std::endl;
