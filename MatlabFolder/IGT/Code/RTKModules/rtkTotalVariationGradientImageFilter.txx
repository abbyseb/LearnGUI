...
#ifndef __rtkTotalVariationGradientImageFilter_txx
#define __rtkTotalVariationGradientImageFilter_txx
...
TotalVariationGradientImageFilter<TInputImage, TOutputImage>
...
::ThreadedGenerateData(const typename TInputImage::RegionType& outputRegionForThread, typename itk::ThreadIdType itkNotUsed(threadID))
{
  ...
  float tvgrad;
  std::vector< std::vector< float > > diff ( NDim+1, std::vector<float> (NDim, 0) );
  std::vector< float > sqrtvalues (NDim+1, 0);
  float cvalue;
  ...
  for (unsigned int k = 0; k < NDim+1; ++k)
	for (unsigned int l = 0; l < NDim; ++l)
	  w[k][l] = 1. / m_SpacingCoeffs[dimsToProcess[l]];
  ...
  while(!oit.IsAtEnd())
    ...
    for (unsigned int l = 0; l < dimsToProcess.size(); l++)
      {
	  diff[0][l] = (cvalue - iit.GetPixel(c - strides[dimsToProcess[l]])) * w[0][l];
      }
    ...
    for (unsigned int k = 0; k < dimsToProcess.size(); k++)
	  ...
	  for (unsigned int l = 0; l < dimsToProcess.size(); l++)
	    {
		diff[k+1][l] = (cvalue - iit.GetPixel(c + strides[dimsToProcess[k]] - strides[dimsToProcess[l]])) * w[k+1][l];
	    }
      }
	...
	for (unsigned int k = 0; k < dimsToProcess.size() + 1; k++)
	  ...
	  for (unsigned int l = 0; l < dimsToProcess.size(); l++)
	    {
		sqrtvalues[k] += diff[k][l] * diff[k][l];
	    }
	  sqrtvalues[k] = sqrtf(sqrtvalues[k] + eps);
	  }

	// Put everything together
	for (unsigned int l = 0; l < dimsToProcess.size(); l++)
	  {
	  tvgrad += diff[0][l] * w[0][l];
	  }
	tvgrad /= sqrtvalues[0];
	for (unsigned int k = 1; k < dimsToProcess.size() + 1; k++)
	  {
	  tvgrad -= diff[k][k-1] / sqrtvalues[k] * w[k][k-1];
	  }
    oit.Set(tvgrad);
    ++oit;
    ++iit;
