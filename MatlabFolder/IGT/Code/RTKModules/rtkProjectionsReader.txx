...
// Customized attenuation format (Andy)
  ...
  if(m_ImageIO != imageIO)
    ...
    if( !strcmp(imageIO->GetNameOfClass(), "HndImageIO") )
	  ...
	  if(!m_ProjectionConversion)
	    {
		typedef itk::CastImageFilter< InputImageType, OutputImageType >  CastImageType;
		typename CastImageType::Pointer castFilter = CastImageType::New();
		castFilter->SetInput(reader->GetOutput());
        m_RawToProjectionsFilter = castFilter;
		}
	  else
	    {
      ...
      // here we update the reader so that the metadata is available.
      m_RawDataReader->UpdateOutputInformation();
      typedef itk::MetaDataObject< double > MetaDataDoubleType;
      const double norm =
        dynamic_cast<MetaDataDoubleType *>(imageIO->GetMetaDataDictionary()["dCTNormChamber"].GetPointer())->GetMetaDataObjectValue();
	  ...
	  if(!m_ProjectionConversion)
        ...
        typename RawFilterType::Pointer rawFilter = RawFilterType::New();
        rawFilter->SetInput( reader->GetOutput() );
        rawFilter->SetNormChamberValue( static_cast<typename OutputImageType::PixelType>(norm) );
        m_RawToProjectionsFilter = rawFilter;
		}
      }
      ...
      typedef itk::MetaDataObject< double > MetaDataDoubleType;
      const double norm =
        dynamic_cast<MetaDataDoubleType *>(imageIO->GetMetaDataDictionary()["ikVNormChamber"].GetPointer())->GetMetaDataObjectValue();
