...
::ThreadedGenerateData(const OutputImageRegionType& outputRegionForThread, typename itk::ThreadIdType itkNotUsed(threadId) )
{
  ...
  // Debug purpose - Andy Shieh 2014
  // std::cout << "Max gap: " << angularGaps[maxAngularGapPos] << std::endl;
  ...
  if(m_Geometry->GetShortScanAngle() < M_PI/2)
    delta = m_Geometry->GetShortScanAngle();
  
  // Debug purpose - Andy Shieh 2014
  // std::cout << "First angle: " << firstAngle << std::endl;
  // std::cout << "Last angle: " << lastAngle << std::endl;
  ...
  double halfDetectorWidth = std::abs( m_Geometry->ToUntiltedCoordinateAtIsocenter(0, corner1[0]) );
  ...
  for(unsigned int k=0; k<outputRegionForThread.GetSize(2); k++)
	...
	// Change -firstAngle to -(lastAngle - M_PI - 2.*delta), which would be the same as 
    ...
    beta = beta - ( lastAngle - M_PI - 2.*delta );
    if (beta<0)
      beta += 2*M_PI;
